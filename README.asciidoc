...
-------------------------------------
pLisp is available for Linux, OS X and Windows (using MSYS2).
...
-----------------------
pLisp is installed by the standard './configure', 'make' and 'sudo make install' commands in the directory where you downloaded/extracted the pLisp files.
